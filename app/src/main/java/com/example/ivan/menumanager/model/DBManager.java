...
public class DBManager extends SQLiteOpenHelper {
    ...
    private static final String CREATE_RECIPE = "CREATE TABLE " + RECIPE + "(" + ID + " INTEGER PRIMARY KEY," + RECIPE_API_STRING + " INTEGER, " + NAME + " TEXT, " + IMAGE + " TEXT, " + DESCRIPTION + " TEXT);";
    ...
    //TODO
    public void removeProduct(String productName, double quantity) {
        int productID = 0;
        int householdID = 0;
        ...
        if(households.get(currentHousehold).getProducts().get(productName).getQuantity() == quantity){
            ourInstance.getWritableDatabase()
                    .delete(HOUSEHOLD_PRODUCT,
            ...
            if (households.get(currentHousehold).removeProduct(productName)) {
                Toast.makeText(context, productName + " has been removed", Toast.LENGTH_SHORT).show();
            }
        }
        else{
            ContentValues contentValues = new ContentValues();
            double newQuantity = households.get(currentHousehold).getProducts().get(productName).getQuantity() - quantity;
            contentValues.put(QUANTITY, newQuantity);
            ourInstance.getWritableDatabase().update(HOUSEHOLD_PRODUCT, contentValues,
                    (HOUSEHOLD_ID + " LIKE ? AND " + PRODUCT_ID + " LIKE ? "), new String[]{String.valueOf(householdID), String.valueOf(productID)});
            contentValues.clear();
            if(households.get(currentHousehold).updateProduct(productName, quantity)) {
                Toast.makeText(context, productName + " has been updated successfully", Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private static void loadInfo() {
        ...
        while (cursorHh.moveToNext()) {
            ...
            while (cursorHh_Pr.moveToNext()) {
                ...
                quantityPr = cursorHh_Pr.getDouble(cursorHh_Pr.getColumnIndex(QUANTITY));
    ...
    private void insertPredefinedValues(SQLiteDatabase db) {
        ...
        for (int i = 1; i < categories.length; i++) {
            ...
            contentValues.put(IMAGE, images[i - 1]);
            db.insert(CATEGORY, null, contentValues);
            contentValues.clear();
        }
